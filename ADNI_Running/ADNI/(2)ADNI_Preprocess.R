...
load("./RGset_ADNI_1919.rda")


pd <- pData(RGset)
qcReport(RGset, sampNames = pd$rownames, sampGroups = NULL, pdf = "./figures/qcReport_ADNI_No_normalize.pdf")
...
ID_1919<-object@colData@rownames
writeMat("./ID_1919.mat",ID_1919=ID_1919)

############### align the ID
ID_1919_logic<-readMat("ID_1919_logic.mat", header = TRUE)
ID_1919_logic<-ID_1919_logic$ID.1919.logic
ID_1919_logic[,2]
De <- (ID_1919_logic[,2]==0) 
sum(De)

tab <- ID_1919[De]
write.csv(tab,file="./tables/Bad_samples_ID_1919.csv")



ID_1905<-ID_1919[!De]
writeMat("./ID_1905.mat",ID_1905=ID_1905)

RGset <- RGset[,!De]
save(RGset,file="./RGset_ADNI_1905.rda") 

############# load RGset_ADNI_1905
load("./RGset_ADNI_1905.rda")
object=preprocessIllumina(RGset) # preprocess 
object<-mapToGenome(object)
object=ratioConvert(object, type="Illumina")#convert raw methylation data
beta <- getBeta(object) #get the beta value for each probe
...
################################### PCs_gender_effect
	...
	################################### PCs_slide_effect
	ID_1905<-object@colData@rownames
	ID_slide<-substr(ID_1905,1,12)
...
data_self<-readMat("gender_self_order.mat", header = TRUE)
gender_self_order<-data_self$gender.self.order
gender_self<-gender_self_order[,2]+1
tabulate(gender_self)
...
tab <- ID_1905[De]
write.csv(tab,file="./tables/Bad_samples1905_final_QC.csv")

########## Remove the bad sample and save RGset
RGset <- RGset[,!De]
gender_self<-gender_self[!De]
ID_slide<-ID_slide[!De]
ID_long_final<-ID_1905[!De]
ID_short_1905<-gender_self_order[,1]
ID_short_final<-ID_short_1905[!De]
...
Cohort <- "ADNI" #Please change to your own cohort name.
mydist=dist(t(beta[sample(1:nrow(beta),10000),]))
mds=cmdscale(mydist)
#pd=pData(dat)
