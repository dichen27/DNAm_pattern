...
order_consensus_14[1]
Clust[order_consensus_14[1]]
...
#################### Enrichment
## read num_CpGs_in_module
result<-readMat("./(Reply1-10)num_CpGs_in_module.mat")
num_CpGs_in_module<-result$num.CpGs.in.module
...
## first run k=1,then run k=3:18, because k=2 only has one module
for (k in 3:length(unique(Clust))) {

print(paste("k=",k,sep = "")) # important function paste
cluster_k <- num[Clust==k]
length(cluster_k)

### dele the largest module
cluster_k_final<-cluster_k[-1]
length(cluster_k_final)

### find sig_cpg_IMAGEN
num_CpG_k<-0
sig_cpg_IMAGEN<-""
for (i in 1:length(cluster_k_final)) {

cmd<-cluster_k_final[i]
...
#topGSA(gst,n=10) # select the show num
topGSA(gst,n=10)

#### save GO
gst_dir<-paste(getwd(),"/Enrichment_Stability/Enrichment_Second_Round_GO_k",k,".rds",sep = "")
...
#### save KEGG
kegg_dir<-paste(getwd(),"/Enrichment_Stability/Enrichment_Second_Round_KEGG_k",k,".rds",sep = "")
