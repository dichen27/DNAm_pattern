...
ME_module190_substract_19_14 <- read.csv('./module190/ME_module190_substract_19_14.csv')
 

r_substract <- read.csv('./module190/r_substract.csv')      
...
ME_module190<-ME_module190_substract_19_14
IMAGEN_weight_order<-matrix(data=NA, nrow =506, ncol =190, byrow = FALSE, dimnames = NULL)
for (i in 1:190){

  k<-order_sampleTree[i]
  IMAGEN_weight_order[,i]<-ME_module190[,k]
}
...
###########################################################################
# Heatmap age-14 n=506
ME_module190<-read.csv('./module190/ME_module190_14.csv')
IMAGEN_weight_order<-matrix(data=NA, nrow =506, ncol =190, byrow = FALSE, dimnames = NULL)
for (i in 1:190){
...
r_map<- cor(IMAGEN_weight_order, method = "pearson", use = "complete.obs") 
pheatmap(r_map, cluster_rows = F,cluster_cols = F,show_rownames = F,show_colnames = F,border_color = "white",color = colorRampPalette(colors = c("#5EAD83","white","#9F558E"))(100))
# mantel
mantel(xdis = r_map, ydis = r_substract_19_14_order, method = "pearson", permutations = 10000, na.rm = TRUE)

###########################################################################
# Heatmap age-19 n=506
ME_module190<-read.csv('./module190/ME_module190_19.csv')
IMAGEN_weight_order<-matrix(data=NA, nrow =506, ncol =190, byrow = FALSE, dimnames = NULL)
for (i in 1:190){
...
###########################################################################
# Heatmap change 450k
ME_module190<-read.csv('./module190/ME_module190_substract_19_14_450k.csv')
IMAGEN_weight_order<-matrix(data=NA, nrow =445, ncol =190, byrow = FALSE, dimnames = NULL)
for (i in 1:190){
...
# mantel
mantel(xdis = r_change_450k, ydis = r_substract_19_14_order, method = "pearson", permutations = 10000, na.rm = TRUE)

###########################################################################
# Heatmap change 850k
ME_module190<-read.csv('./module190/ME_module190_substract_19_14_850k.csv')
IMAGEN_weight_order<-matrix(data=NA, nrow =61, ncol =190, byrow = FALSE, dimnames = NULL)
for (i in 1:190){
